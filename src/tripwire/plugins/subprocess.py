...
from tripwire._errors import ConflictError, UnmockedInteractionError
...
# Set during _install_patches so _check_conflicts can distinguish tripwire
# patches from foreign patches during nested sandbox activations.
# ---------------------------------------------------------------------------

_tripwire_subprocess_run: Any = None
_tripwire_shutil_which: Any = None
...
def _find_subprocess_plugin(verifier: "StrictVerifier") -> "SubprocessPlugin":
    try:
        ...
        raise RuntimeError(
            "BUG: tripwire SubprocessPlugin interceptor is active but no "
...
class SubprocessPlugin(BasePlugin):
    ...
    _original_subprocess_run: ClassVar[Callable[..., Any] | None] = None
    _original_shutil_which: ClassVar[Callable[..., Any] | None] = None
    ...
    def install_patches(self) -> None:
        ...
        _tripwire_subprocess_run = _run_interceptor
        _tripwire_shutil_which = _which_interceptor

        subprocess.run = _run_interceptor
        setattr(shutil, "which", _which_interceptor)
    ...
    def restore_patches(self) -> None:
        ...
        _tripwire_subprocess_run = None
        _tripwire_shutil_which = None
    ...
    def _handle_run(self, *args: Any, **kwargs: Any) -> "subprocess.CompletedProcess[str]":  # noqa: ANN401
        ...
        if isinstance(cmd, str):
            raise TypeError(
                ...
                f"tripwire requires a list of arguments, e.g. {[cmd]!r}. "

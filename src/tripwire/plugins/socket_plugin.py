...
from tripwire._context import GuardPassThrough, get_active_verifier, get_verifier_or_raise
from tripwire._firewall_request import SocketFirewallRequest
from tripwire._state_machine_plugin import StateMachinePlugin, _StepSentinel
...
class SocketPlugin(StateMachinePlugin):
    ...
    def format_mock_hint(self, interaction: Interaction) -> str:
        sid = interaction.source_id
        method = sid.split(":", 1)[-1] if ":" in sid else sid
        return f"    tripwire.socket_mock.new_session().expect({method!r}, returns=...)"
    ...
    def format_unmocked_hint(
        ...
        return (
            ...
            f"    tripwire.socket_mock.new_session().expect({method!r}, returns=...)"
        )

    def format_assert_hint(self, interaction: Interaction) -> str:
        sm = "tripwire.socket_mock"
        sid = interaction.source_id
        if sid == _SOURCE_CONNECT:

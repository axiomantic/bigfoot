...
def _get_jwt_plugin() -> JwtPlugin:
    ...
    for plugin in verifier._plugins:
        if isinstance(plugin, JwtPlugin):
            return plugin
    raise RuntimeError(
        "BUG: tripwire JwtPlugin interceptor is active but no "
...
class JwtPlugin(BasePlugin):
    ...
    def format_mock_hint(self, interaction: Interaction) -> str:
        source_id = interaction.source_id
        operation = source_id.split(":", 1)[-1] if ":" in source_id else "?"
        return f"    tripwire.jwt.mock_{operation}(returns=...)"
    ...
    def format_unmocked_hint(
        ...
        return (
            ...
            f"    tripwire.jwt.mock_{operation}(returns=...)"
        )
    ...
    def format_assert_hint(self, interaction: Interaction) -> str:
        ...
        lines = "\n".join(parts)
        return (
            f"    tripwire.jwt.assert_{operation}(\n"
            f"{lines}\n"
            f"    )"
        )

...
# Set during _install_patches so _check_conflicts can distinguish tripwire's
# interceptors from foreign patchers during nested sandbox activations.
# ---------------------------------------------------------------------------

_tripwire_create_subprocess_exec: Callable[..., Any] | None = None
_tripwire_create_subprocess_shell: Callable[..., Any] | None = None
...
def _find_async_subprocess_plugin(
    ...
    for plugin in verifier._plugins:
        if isinstance(plugin, AsyncSubprocessPlugin):
            return plugin
    raise RuntimeError(
        "BUG: tripwire AsyncSubprocessPlugin interceptor is active but no "

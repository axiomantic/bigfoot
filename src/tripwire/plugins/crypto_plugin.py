...
def _get_crypto_plugin() -> CryptoPlugin:
    ...
    for plugin in verifier._plugins:
        if isinstance(plugin, CryptoPlugin):
            return plugin
    raise RuntimeError(
        "BUG: tripwire CryptoPlugin interceptor is active but no "
...
class CryptoPlugin(BasePlugin):
    ...
    def format_mock_hint(self, interaction: Interaction) -> str:
        ...
        return f"    tripwire.crypto.{mock_name}(returns=...)"
    ...
    def format_unmocked_hint(
        ...
        return (
            ...
            f"    tripwire.crypto.{mock_name}(returns=...)"
        )

...
class MethodProxy:
    ...
    Calling it routes through the tripwire interceptor.
    """
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:  # noqa: ANN401
        """Called when the mock is invoked. Routes through tripwire interceptor."""
...
class ImportSiteMock(_BaseMock):
    """A mock registered via tripwire.mock("mod:attr")."""
    ...
    def _resolve_target(self) -> tuple[object, str]:
        from tripwire._path_resolution import resolve_target  # noqa: PLC0415
...
class ObjectMock(_BaseMock):
    """A mock registered via tripwire.mock.object(target, "attr")."""
    ...
    def _register_active_patch(
        ...
        if patch_key in self._active_patches:
            ...
            raise ConflictError(
                ...
                patcher=f"tripwire mock ({existing._display_name})",
            )

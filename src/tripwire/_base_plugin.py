"""BasePlugin abstract base class for all tripwire plugins."""
...
class BasePlugin(ABC):
    """Abstract base for all tripwire plugins.
    ...
    4. Register via entry_points in pyproject.toml or [tool.tripwire]
    5. Use ``with tripwire:`` in tests (never verifier.sandbox() directly)

    Import from tripwire directly:
    ...
    def config_key(cls) -> str | None:
        """Return the [tool.tripwire.<key>] section name for this plugin.
        ...
        Example: HttpPlugin returns "http", mapping to [tool.tripwire.http].
        """
        return None

    def load_config(self, config: dict[str, Any]) -> None:
        """Apply configuration from the plugin's [tool.tripwire.<key>] sub-table.

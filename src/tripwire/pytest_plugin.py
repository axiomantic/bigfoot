# src/tripwire/pytest_plugin.py
"""pytest fixture registration for tripwire."""
...
def pytest_configure(config: pytest.Config) -> None:
    """Register tripwire pytest markers and install context propagation."""
...
def _tripwire_auto_verifier() -> Generator[StrictVerifier, None, None]:
    ...
    activated -- the test (or module-level tripwire.sandbox()) controls sandbox lifetime.
    """
...
@pytest.fixture
def tripwire_verifier(_tripwire_auto_verifier: StrictVerifier) -> StrictVerifier:
    """Explicit fixture for tests that need direct access to the verifier.

    Usage:
        def test_something(tripwire_verifier):
            http = HttpPlugin(tripwire_verifier)
            http.mock_response("GET", "https://api.example.com/data", json={})
            with tripwire_verifier.sandbox():
                response = httpx.get("https://api.example.com/data")
                tripwire_verifier.assert_interaction(http.request, method="GET")
    """
    return _tripwire_auto_verifier


@pytest.fixture(autouse=True, scope="session")
def _tripwire_guard_patches() -> Generator[None, None, None]:
...
def pytest_runtest_call(item: pytest.Item) -> Generator[None, None, None]:
    ...
    is handled by ``_tripwire_guard_patches`` (session-scoped). Per-test
    ...
    if "guard_allow" in config:
        ...
        raise TripwireConfigError(
            "The guard_allow config key has been replaced by [tool.tripwire.firewall]. "
            "Migration: guard_allow = [\"http\", \"dns\"] becomes "
            "[tool.tripwire.firewall]\\nallow = [\"http:*\", \"dns:*\"]"
        )

    from tripwire._firewall import (  # noqa: PLC0415
        Disposition,
        FirewallRule,
        _firewall_stack,
    )
    from tripwire._match import M  # noqa: PLC0415

    frames: list[FirewallRule] = []

    # ---------------------------------------------------------------
    # Layer 0: Global TOML rules from [tool.tripwire.firewall]
    # ---------------------------------------------------------------
    ...
    # Structured protocol sections (e.g., [tool.tripwire.firewall.http])
...
def _path_matches_glob(test_path: str, glob_pattern: str) -> bool:
    ...
    return tripwire_match(glob_pattern, test_path)

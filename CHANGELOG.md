...
- `tripwire.allow(...)`, `tripwire.deny(...)`, and `tripwire.restrict(...)` now raise `TripwireError` when called outside any active sandbox, with a message pointing the user at `[tool.tripwire.firewall]` for module-scoped rules.
...
- Rewrite README for human voice: add Morpheus quote, rewrite intro, fix LLM-sounding prose, break out Firewall mode section, correct allow/restrict examples to use context manager syntax
